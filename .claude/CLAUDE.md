...
サイトには似た名前の2つの詳細ページがあり、**これらは全く別のページです：**

#### 1. `/detail?id=投稿ID` - 個別投稿の詳細ページ
- **データソース**: `posts-YYYY-MM.json`（月別ファイル）
- **URL例**: `/detail?id=1763466433-3`
- **表示内容**: 1つの投稿の詳細（画像、キャプション、日付など）
- **使用スクリプト**: `detail-app.js`

#### 2. `/gallery/detail?id=品種ID` - 品種別の詳細ページ
- **データソース**: `species/ジサクボイザナギ.json`（品種ファイル）
- **URL例**: `/gallery/detail?id=ジサクボイザナギ`
- **表示内容**: 1つの品種の全投稿一覧
- **使用ファイル**: `src/pages/gallery/detail.astro`

**⚠️ 重要：** ユーザーが「詳細ページが開けない」と言った場合：
- まずどちらのページか確認する
- `/detail` なら月別JSONファイルの問題
- `/gallery/detail` なら品種JSONファイルの問題
